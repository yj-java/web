...
public class SecKill {
     ...
     * 1.2.3：redisson实现了上述逻辑，可以直接使用，getlock,unlock
     * 1.2.3.1 redisson加锁时还可以对库存进行分段，比如将1000个库存分成100份，每份分别加锁
     * 1.2.4：redis单机情况下redisson可以锁成功，但是redis集群情况下，主从同步的时候可能会因为主库挂掉之前没来得及同步锁信息而导致锁失效，所以需要引入redlock，需要一半以上redis实例
